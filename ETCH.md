...
Etch captures the metadata of agentic work — prompts, sessions, workflow versions, orchestration traces — and binds it to git commits so the next AI agent in a codebase has rich context about how past agents and humans worked there.

## Etymology

A *etch* is a stack of stones left by past travelers to mark the path for those who follow. Each agent leaves a marker on each commit; future agents read the markers to navigate the territory.

## Philosophy

**The user is the next agent.** Etch's primary consumer is not a human opening a dashboard. It is the next AI agent starting work in a codebase, reading prior context to inform its decisions. Humans benefit through their agents — the agent surfaces patterns from Etch during normal conversation.

**Nothing changes in the operator's workflow.** Capture happens via git hooks fired by the operator's existing actions (`git commit`, `git push`, branch creation, merge, rebase). No launcher wrapper, no special invocation, no `etch-claude` instead of `claude`. The operator does what they already do; Etch captures invisibly.

**Git is the substrate.** Etch runs on bare git. No forge required. Forge integrations (PR state, CI outcomes) are deferred — v0 derives everything from git alone.

**Workflow is a first-class artifact.** Most prior art binds metadata to *code* (the diff, the commit). Etch additionally binds to the *workflow* — which orchestrator at which version produced this run. Workflows iterate; the version that produced a successful PR is recoverable and comparable.

**The workflow slot is open-ended.** Workflows vary wildly across teams (`lattice-orchestrator`, Devin-style, plan→implement→review, RAG-driven, eval-gated, hand-rolled bash) and the shape evolves monthly. Etch provides the binding slot; the workflow author fills it with whatever they want recorded. Property bag, not fixed schema.

**Dumb CLI, smart consumer.** Etch's CLI is a data layer that returns structured matches. The agent reading the data does the reasoning. No embedded LLM in Etch; the consuming agent is the intelligence.
...
Etch's strategy: **emit Agent Trace records as a side-channel output of every capture.** Whatever Etch records internally also serializes to `agent-trace.json`. The Cursor / Cognition / Anthropic toolchains can read what Etch produces for the cost of one serializer. Free interop; zero negotiation.

Etch owns the layer Agent Trace leaves open: prompt corpus, workflow versioning, derived outcomes, multi-agent attribution. Don't compete with Agent Trace. Ride it.

[Contextual Commits](https://github.com/berserkdisruptors/contextual-commits) (typed action lines in commit bodies: `intent`, `decision`, `rejected`, `constraint`, `learned`) is a second emerging standard at the in-commit-body layer. Etch supports emitting these as an opt-in per-repo setting — capture is invisible by default; commit-body mutation is not silent.

## Architecture: three surfaces

| Surface | Lives | Fires on | Purpose |
|---|---|---|---|
| **Git hooks** | `.git/hooks/` via `core.hooksPath → .etch/hooks/` (checked-in) | `post-commit`, `post-checkout`, `pre-push`, `post-merge`, `post-rewrite` | Capture — write bindings, slice sessions |
...
| **SKILL.md** | `.claude/skills/etch/` (and per-agent equivalents) | Agent reads at session start | Consumption — query Etch, surface patterns to operator |
...
`etch install` wires all three: sets `git config core.hooksPath`, edits agent settings files, drops the skill into the project.

## Storage

| Tier | Lives | What goes here |
|---|---|---|
| **Working state** | `.etch/` in the project (checked into git) | bindings, hooks, config, locally-rendered cooked artifacts. Operator-inspectable, version-controlled. |
| **Sync** | `refs/notes/etch` (git notes namespace) | bindings serialized for cross-machine portability and standard-tool interop. Written on `pre-push`. |
| **Source pointers** | `~/.claude/projects/.../<id>.jsonl`, `~/.codex/sessions/...`, etc. | Raw session JSONLs, untouched, pointed to from bindings. |
| **Pinned (on demand)** | `.etch/pinned/<sha>/` or a content-addressed store | Archival copies of sessions referenced by long-lived bindings. `etch pin` triggers. |
...
**The `workflow` field is a property bag.** Etch does not define its keys. The workflow author records whatever is meaningful — phase, ticket, sub-agent role, model used for review, retry count, eval gate result, arbitrary nested objects. Etch stores it; queries index across whatever keys exist.
...
Runs are derived, not declared. Etch does not own a run lifecycle.
...
Before `git commit` runs, the agent's `PreToolUse` hook fires (Claude Code; equivalents on Codex / OpenCode). The hook writes `~/.etch/active/<session-id>.json` with `session_id`, `cwd`, `branch`, `pid`, and the session's current event-index. The git `post-commit` hook reads the most recent marker for this `cwd` + `branch` and binds.
...
**Marker lifecycle.** Marker files are cheap, but at 60+ concurrent sessions the `~/.etch/active/` directory needs maintenance. Cleanup triggers:
...
| `pre-push` | Sync bindings to `refs/notes/etch` for remote portability |
...
- Pointer by default. Etch stores `~/.claude/projects/.../<id>.jsonl` paths, not copies.
- Slice is `(session_id, from_idx, to_idx)` — bounded by `git commit` tool-call events.
- `etch pin <commit-or-binding>` copies the session JSONL to a content-addressed store for archival.

## Consumption

Agent reads SKILL.md at session start. Skill tells the agent to:

1. Run `etch context` once at session start; digest the returned structured data.
2. **Selectively surface** relevant patterns to the operator — only when a surfaced pattern would change a decision the operator is about to make. Not chatty. Not silent. *"Etch noticed prompts shaped like this one tend to take three review cycles — want to rephrase?"* is appropriate. Constant background commentary is not.
3. Query `etch query <question>` when relevant prior work would inform the current decision.

The CLI is dumb: takes a question or filter, returns structured matches. The agent reasons over the matches.

## Trust and visibility

Provenance tooling that mutates the substrate silently poisons the substrate. (See the VS Code Copilot Co-Author backlash, [Penligent writeup](https://www.penligent.ai/hackinglabs/vs-code-copilot-co-author-when-ai-attribution-becomes-a-supply-chain-trust-problem/).) Etch's posture:

- **Capture is explicit opt-in at the repo level.** Running `etch install` is the consent gesture. Until then, nothing fires.
- **Bindings are operator-inspectable.** `.etch/bindings.jsonl` lives in the repo, version-controlled, readable in any text editor.
- **`etch inspect [<commit>|<session>|<branch>]`** dumps everything Etch has recorded about a target.
- **`etch redact <binding-id>`** removes a binding, with the redaction itself recorded as an event (audit-trail-preserved).
- **No silent commit-message mutation.** Contextual Commits emission requires an explicit per-repo opt-in.
- **No silent ref-push to remote.** `pre-push` sync to `refs/notes/etch` is on by default but visible in repo config and disabled with one setting.
- **Source pointers, not silent copies.** Session JSONLs stay where the agent put them. `etch pin` is an explicit operation.

## What v0 ships

- `etch install` — wires git hooks, agent settings, and the SKILL.md
- Five git hooks with marker-file + process-tree-walk attribution
- `PreToolUse` / `PostToolUse` hooks for Claude Code, Codex, OpenCode
- Per-commit binding format (jsonl in `.etch/bindings.jsonl`)
- Open-ended `workflow` property bag
- **Agent Trace emission** — every binding serializes to an `agent-trace.json` sidecar
- Sidecar (`.etch/`) + git-notes-on-push storage
- `etch context` and `etch query` CLI subcommands
- `etch inspect` and `etch redact` operator-control subcommands
...
- MCP tool surface (`mcp__etch__*`)
- Cross-machine portability of pinned sessions
- Meta-feedback loop: Etch-side analysis that pings the operator with prompt-pattern coaching (v2)
...
- **[Agent Trace](https://github.com/cursor/agent-trace)** — vendor-neutral attribution schema. Cognition / Cursor / 17 partners (Anthropic, OpenAI, Google, Vercel, Cloudflare, …). Storage-agnostic; punts on prompts, quality, workflow. Etch emits this.
- **[Contextual Commits](https://github.com/berserkdisruptors/contextual-commits)** — typed action lines (`intent` / `decision` / `rejected` / `constraint` / `learned`) in commit bodies. Reference impl as Claude Code skills. Etch emits this opt-in.
...
- **[GitHub Agentic Workflows](https://github.github.com/gh-aw/)** — workflow-as-branch with phase commits as state checkpoints. Convergent with Etch's "workflow is versioned" thesis.
...
**Convergence observed:** the field has fragmented into three storage patterns (git notes, sidecar dotfolders, parallel branches). Agent Trace is the standard for *what* to record; the *where* remains pluralistic. Etch picks sidecar-plus-notes for working state + sync.

## Differentiation hypothesis

What Etch adds over the prior art:

1. **Workflow as a versioned first-class artifact with an open-ended property bag.** Git AI binds to code; h5i binds to reasoning; Agent Trace binds to attribution. None model the workflow blueprint itself as a versioned, queryable entity with a flexible schema. Etch does.
