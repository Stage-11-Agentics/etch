...
Etch captures orchestration metadata from `CAIRN_*` environment variables at
...
- Commit: `d44c94889c1dec5e1f0be62f5483cccc78c9bace`
  (branch `feat/etch-12-cairn-skill-export`, remote
  `github.com/Stage-11-Agentics/c11`)

The change adds the `CAIRN_*` export block to all three delegator boot templates
...
contract, and per-sub-agent `CAIRN_AGENT_ROLE` / `CAIRN_PARENT_SESSION_ID` on the
...
| Var | → field | Notes |
|---|---|---|
| `CAIRN_ORCHESTRATOR_TYPE` | `orchestration.type` | defaults to `"manual"` when absent |
| `CAIRN_DISPATCH_METHOD` | `orchestration.dispatch_method` | `c11_delegator` / `headless_clear` / … |
| `CAIRN_TICKET_ID` | `orchestration.ticket_id` | |
| `CAIRN_RUN_ID` | `orchestration.run_id` | groups a run; one ULID across all delegators |
| `CAIRN_AGENT_ROLE` | `orchestration.role` | `delegator` / `planner` / `impl` / `reviewer` / `fix` |
| `CAIRN_WORKFLOW_VERSION` | `orchestration.workflow_version` | git short SHA of the skill |
| `CAIRN_ORCHESTRATION_EXTRA` | `orchestration.extra` | JSON string → open property bag |
| `CAIRN_PARENT_SESSION_ID` | `session.parent_session_id` | consumed at the hook layer, not inside `Orchestration` |

`CaptureOrchestration()` reads the first seven (the six `orchestration.*` fields
plus `extra`). `CAIRN_PARENT_SESSION_ID` is handled in `session_start.go` and lands
