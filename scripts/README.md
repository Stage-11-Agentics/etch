...
the `cairn` agent against the **real** `entire` CLI, then simulates a full agent
session by piping hook-event JSON through the binary the same way Entire's hooks
do. It asserts that a single immutable ref appears under `refs/cairn/sessions/`,
that its `session.json` carries `schema_version: cairn.session.v1`, and that the
